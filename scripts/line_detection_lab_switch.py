...
class LineFollower:
    def __init__(self):
        ...
        if self.work_mode == 'simulation':
            ...
            self.lower_LAB_d = np.array(eval(rospy.get_param('~lower_LAB_d')))
            self.upper_LAB_d = np.array(eval(rospy.get_param('~upper_LAB_d')))
    ...
    def camera_callback(self, image):
        ...
        if self.work_mode == 'simulation':
            ...
            lab1 = cv2.cvtColor(img_raw, cv2.COLOR_BGR2LAB)
            # Threshold the LAB image to get only specific colors
            mask = cv2.inRange(lab, self.lower_LAB, self.upper_LAB) 
            mask1 = cv2.inRange(lab, self.lower_LAB_d, self.upper_LAB_d) 

        # Detect first line centroid
        while self.detectedline == 0:
